...
#   by only allowing the specific bidirectional traffic configured by the user.
#
# by Psycho (@UDPsycho)
...
echo -e "\n${YELLOW}[i]${RESET} Applying firewall rules...\n"
    ...
    message="    ${GREEN}[+]${RESET} ${DIM}Deleting${RESET} previous rules and counters."
    display_config_message "$message"
    delete_rules

    message="    ${GREEN}[+]${RESET} ${DIM}Setting${RESET} default policies (${RED}$POLICY${RESET})."
    display_config_message "$message"
    set_default_policies

    message="    ${GREEN}[+]${RESET} ${DIM}Allowing${RESET} any traffic from localhost to localhost (${L_CYAN}$LOCALHOST${RESET})."
    display_config_message "$message"
    allow_localhost

    message="    ${GREEN}[+]${RESET} ${DIM}Allowing${RESET} any traffic from the local IP to local IP (${L_CYAN}$LOCAL_IP${RESET})."
    display_config_message "$message"
    allow_local_IP
